...
class TodayPlan extends Component {
    ...
    componentDidMount() {
        axios.get(config.dbUrl + "plans/get").then((res) => {
            let tmpLst = []
            let today = this.convertDate(Date.now());
            res.data.map((obj, i)=>{
                if(today == this.convertDate(obj.date))
                    tmpLst.push(obj);
            })
            this.setState({ plansList: tmpLst }, () => {
...
export default withStyles(higherOrderClass, { withTheme: true })(TodayPlan);